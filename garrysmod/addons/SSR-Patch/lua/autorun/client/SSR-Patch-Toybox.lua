...
hook.Add("HUDPaint", "lToybox", function()
    ...
    for k, v in pairs(Downloads) do
        if v.f < 0.99 then
            UpdatePackageDownloadStatus(v.id, v.filename, v.f + (FrameTime() * v.speed), "", v.size)
        end
    end
end)

usermessage.Hook("lToyboxloadCode", function(data)
    lToyboxloadCode(data:ReadString(), data:ReadString(), data:ReadFloat())
end)

usermessage.Hook("lToyboxDownload", function(data)
    UpdatePackageDownloadStatus(data:ReadFloat(), data:ReadString(), data:ReadFloat(), data:ReadString(), data:ReadFloat())
end)
...
function UpdatePackageDownloadStatus( id, name, f, status, size )
    ...
    if ( dl == nil ) then
        ...
        dl:SetAlpha( 10 )
        dl.speed = math.Remap(math.Clamp(size / 800000, 0.5, 4), 0, 6, 0.6, 0)
        dl.size = size
        Downloads[ id ]     = dl
        Main:Add( dl )
